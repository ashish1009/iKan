...
namespace ikan {
  ...
  /// Class to dispatch event dynamically. Event type can be checked with current event and perform the functionality passed as funciton pointer
  ...
  public:
    ...
    ///  - Once an event callback triggered, it check the triggered event's type( with base calss 'Even') with the explicit event type 'T'.
    ///    If same then executes  the Function F (Function pointer passed to Dispatcher)
    ///  - Important: Event Should be Dispached using this function only. All Event handler takes only base Instance of Event. This Dispatcher
    ///               Dispatches the function binded to a specific Event. So checked all the triggered events at same time and return the calling
    ///               on Function Binded as Funciton pointer <F>
