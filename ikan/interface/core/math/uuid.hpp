...
namespace ikan {
  ...
  /// This class generates a "UUID" (universally unique identifier) or GUID is (usually) a 64-bit integer used to "uniquely" identify
  /// information. In Kreator Engine, even though we use the term GUID and UUID, at the moment we're simply using a randomly generated
  /// 64-bit integer, as the possibility of a clash is low enough for now.
  ///  - Important:
