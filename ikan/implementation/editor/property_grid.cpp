...
namespace ikan {
  ...
  bool PropertyGrid::Search(char* value, const char* hint, const glm::vec2& size) {
    ...
    static std::shared_ptr<Texture> search_exture = Renderer::GetTexture(DM::CoreAsset("textures/icons/search.png"));
    PropertyGrid::Image(INT2VOIDP(search_exture->GetRendererID()), size, glm::vec2( 0, 1 ), glm::vec2( 1, 0 ));
  ...
  uint32_t PropertyGrid::ComboDrop(const char* label, const std::vector<std::string>& options, uint32_t current_value, float col_width) {
